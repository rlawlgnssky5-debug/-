...
const PRODUCTS = {
  solo: {
    ...
    steps: [
      ["스튜디오 방문 · 축가 사전 녹음", "예약한 날짜에 위스티아 스튜디오를 방문해 엔지니어의 1:1 디렉팅을 받으며 본식에서 부를 축가를 한 소절씩 녹음합니다", "assets/img/ar-process/self/01-recording.webp"],
      ["AR 속 내 목소리 비율 선택", "30% · 50% · 70% · 100%를 실제 음원으로 비교해 본식에서 가장 편안한 비율을 선택합니다", "assets/img/ar-process/self/03-ratio-casual.webp"],
      ["완성된 AR 음원 제작", "음정과 박자를 자연스럽게 다듬고 예식장에서 바로 재생할 수 있는 AR 음원으로 완성합니다", "assets/img/ar-process/02-mixing.webp"],
  ...
  duo: {
    ...
    steps: [
      ["스튜디오 방문 · 듀엣 축가 사전 녹음", "예약한 날짜에 두 분이 스튜디오를 방문해 파트와 화음을 맞추고 엔지니어의 디렉팅에 따라 한 소절씩 녹음합니다", "assets/img/duet-film/duet-recording.webp"],
      ["AR 속 목소리 비율 선택", "30% · 50% · 70% · 100%를 실제 음원으로 비교해 두 분이 함께 부르기 편한 비율을 선택합니다", "assets/img/ar-process/self/03-ratio-casual.webp"],
      ["완성된 듀엣 AR 음원 제작", "두 보컬의 음정·박자·화음과 밸런스를 다듬어 예식장에서 바로 재생할 수 있는 AR로 완성합니다", "assets/img/ar-process/02-mixing.webp"],
      ["본식에서 함께 부르기", "미리 녹음한 두 분의 목소리와 함께 현장에서 직접 노래합니다", "assets/img/ar-process/04-live.webp"],
      ["최종 검수 & 완성본 전달", "본식에서 바로 사용할 수 있도록 전체 음원을 검수한 뒤 전달합니다", {duration:"약 7일",note:"녹음 완료 후 일정 확정"}]
    ],
  ...
  proposal: {
    ...
    sub: "직접 부른 노래와 전하고 싶은 이야기를 원하는 영상 구성에 맞춰 한 편의 프로포즈 필름으로 완성합니다",
    normal: 200000,
    ...
    steps: [
      ...
      ["스토리 영상 촬영", "인터뷰와 편지, 뮤비 클립 또는 녹음 메이킹 등 선택한 구성에 맞춰 이야기를 촬영합니다"],
    ...
    composition: [
      ["인터뷰", "처음 만난 순간과 함께하고 싶은 이유를 자신의 말로 남깁니다", "assets/img/proposal/process-01.webp"],
      ["립싱크 뮤비 클립", "직접 부른 노래에 맞춰 자연스럽게 부르는 장면을 뮤직비디오처럼 담습니다", "assets/img/proposal/process-02.webp"],
      ["녹음 메이킹 필름", "한 사람만을 위한 노래를 실제로 녹음하는 과정을 담습니다", "assets/img/proposal/process-03.webp"],
      ["녹음 메이킹 필름", "카메라 뒤에서 노래와 영상을 완성해 가는 현장까지 보여드립니다", "assets/img/proposal/process-04.webp"],
...
const AR_PURPOSES = {
  self: {
    ...
    items: [
      ["스튜디오 방문 · 축가 사전 녹음", "예약한 날짜에 위스티아 스튜디오를 방문해 엔지니어의 1:1 디렉팅을 받으며 본식에서 부를 축가를 한 소절씩 나누어 녹음합니다", "assets/img/ar-process/self/01-recording.webp"],
      ["AR 속 내 목소리 비율 선택", "30% · 50% · 70% · 100% 중 원하는 비율을 실제 음원으로 비교할 수 있습니다", "assets/img/ar-process/self/03-ratio-casual.webp"],
  ...
  friend: {
    ...
    items: [
      ["축가 사전 녹음", "결혼식에서 부를 축가를 미리 녹음합니다", "assets/img/ar-process/friend/01-female.webp"],
      ["AR 속 내 목소리 비율 선택", "30% · 50% · 70% · 100% 중 원하는 비율을 실제 음원으로 비교할 수 있습니다", "assets/img/ar-process/friend/03-female.webp"],
      ["완성된 AR 음원 제작", "현장에서 바로 재생할 수 있도록 보컬이 포함된 AR 음원으로 완성합니다", "assets/img/ar-process/friend/02-female.webp"],
      ["친구에게 축가 전하기", "미리 녹음한 내 목소리와 함께 현장에서 직접 노래합니다", "assets/img/ar-process/friend/04-female-live.webp"],
...
const SERVICE_META = {
  ...
  "solo-film":{type:"A SONG FOR YOU",label:"솔로 축가 영상",use:"본식 축가 순서",who:"신랑 또는 신부 한 사람",result:"본식 상영용 솔로 축가 영상 MP4 + 보컬 보정·믹싱을 마친 완성 음원 WAV·MP3",short:"한 사람이 직접 부른 노래를 영상으로",image:"assets/img/solo-film/solo-film-cover-v2.webp"},
...
const FILM_FORMATS = {
  live:{
    title:"뮤비 클립 필름",
    summary:"하객 메시지 + 인터뷰 + 라이브 뮤비 클립 + 서로에게 전하는 편지 구성",
    notes:[]
  },
  making:{
    title:"녹음 메이킹 필름",
    summary:"인서트 컷 + 녹음실 메이킹 컷 구성",
    notes:[
...
const FILM_FORMAT_IMAGES = {
  wedding:{live:"assets/img/wedding/03-lipsync-mv.webp",making:"assets/img/duet-recording-making-retouched-v2.png"},
  "duet-film":{live:"assets/img/duet-film/duet-music-video.webp",making:"assets/img/duet-recording-making-retouched-v2.png"},
...
const FILM_PROCESS_IMAGES = {
  wedding:["assets/img/process/consultation.webp","assets/img/process/vocal-directing.webp","assets/img/wedding/03-lipsync-mv.webp","assets/img/process/melodyne-tuning.webp","assets/img/ar-process/02-mixing.webp","assets/img/process/video-color-grading.webp"],
  "duet-film":["assets/img/process/consultation.webp","assets/img/process/vocal-directing.webp","assets/img/duet-film/duet-music-video.webp","assets/img/process/melodyne-tuning.webp","assets/img/ar-process/02-mixing.webp","assets/img/process/video-color-grading.webp"],
...
const FILM_PRICE_IMAGES = {
  "duet-film":{making:["assets/img/duet-film/making-female.webp","assets/img/duet-film/making-male.webp"]}
}
...
function priceBar(key,purpose=""){return '<div class="fixed-price"><div><span>'+PRODUCTS[key].title+'</span><a class="price-bar-action" href="#/event/'+key+(purpose?'/'+purpose:'')+'"><span><small>선택한 구성의 혜택까지</small><strong>이벤트 적용 가격 확인하기</strong></span><b aria-hidden="true">↗</b></a></div></div>'}
...
function filmProcessSection(productKey,steps){const live=filmProcessSteps(productKey,steps,"live"),making=filmProcessSteps(productKey,steps,"making");return '<section class="shell section film-process-section" id="process">'+heading("","진행 과정","두 가지 필름의 진행 과정을 각각 확인해 주세요")+'<div class="process-type-tabs" role="tablist" aria-label="영상 구성별 진행 과정"><button type="button" role="tab" aria-selected="true" aria-controls="processTypeLive" data-process-format="live"><strong>뮤비 클립 필름</strong></button><button type="button" role="tab" aria-selected="false" aria-controls="processTypeMaking" data-process-format="making"><strong>녹음 메이킹 필름</strong></button></div><div id="processTypeLive" class="process-type-panel" role="tabpanel" data-process-panel="live">'+processList(live)+'</div><div id="processTypeMaking" class="process-type-panel" role="tabpanel" data-process-panel="making" hidden>'+processList(making)+'</div><p class="process-type-note">실제 영상 구성 선택은 가격 확인 화면에서 진행합니다</p></section>'}
...
function renderHome(){
 const groups = [
  {title:"영상으로 준비하기",description:"직접 부른 노래와 두 분의 이야기를 영상으로 완성해 예식이나 고백 순간에 재생합니다",items:[
   {href:"#/detail/wedding",title:"식전 영상 만들기",description:"뮤비 클립 필름 또는 녹음 메이킹 필름으로 두 분의 이야기를 전합니다"},
   {href:"#/film",title:"축가 영상 만들기",description:"직접 부른 노래를 뮤직비디오로 완성해 본식 축가 순서에 상영합니다"},
   {href:"#/detail/proposal",title:"프로포즈 · 답프로포즈 영상 만들기",description:"직접 부른 노래에 원하는 영상 구성을 선택해 한 사람을 위한 고백을 완성합니다"}
  ]},
  {title:"현장에서 직접 부르기 · 사전 녹음",description:"스튜디오에서 사전 녹음한 목소리가 담긴 AR과 함께 예식 당일 직접 축가를 부릅니다",items:[
...
function bookingFilmFormatSection(productKey){return '<section class="booking-film-format"><h2>영상 구성 선택</h2><p>원하시는 영상 구성을 한 가지 선택해 주세요</p><div class="booking-format-grid">'+Object.entries(FILM_FORMATS).map(([key,format])=>'<label class="booking-format-card"><input type="radio" name="filmFormat" data-film-format="'+key+'" '+(selectedFilmFormat===key?'checked':'')+'><span>'+bookingFilmFormatMedia(productKey,key)+'<strong>'+format.title+'</strong><small>'+format.summary+'</small>'+filmFormatNotes(format)+'</span></label>').join("")+'</div></section>'}
function filmSelectionNotice(productKey){return '<aside class="shell film-selection-notice"><div>'+label("TWO FILM STYLES")+'<h2>영상 구성은 가격 확인 화면에서 선택합니다</h2><p>뮤비 클립 필름과 녹음 메이킹 필름 중 원하는 구성을 선택할 수 있습니다</p></div>'+cta("구성·가격 확인하기","#/event/"+productKey,"light")+'</aside>'}
function filmFormatIncluded(productKey){const onePerson=productKey==="proposal"||productKey==="solo-film";const common=onePerson?["1인 레코딩 · 보컬 디렉팅","디테일 음정·박자 보정","보컬 믹싱 · 최종 마스터링"]:["2인 레코딩 · 보컬 디렉팅","파트 · 화음 구성","디테일 음정·박자 보정","보컬 믹싱 · 최종 마스터링"];const format=selectedFilmFormat==="live"?["하객 메시지 촬영","인터뷰 촬영","뮤비 클립 촬영","전하는 편지 촬영"]:["인서트 컷 촬영","녹음실 메이킹 촬영","인트로·아웃트로 타이포"];return [...common,...format,"영상 편집 · 색감 보정","상영용 최종본 · 완성 음원"]}
function bookingPackageList(productKey,p){const items=FILM_FORMAT_PRODUCTS.has(productKey)?filmFormatIncluded(productKey):p.included.map(packageItem);return items.map(x=>'<li><span aria-hidden="true">✓</span><span>'+x+'</span></li>').join("")}
...
function detailResult(p,m,w,song){
 ...
 if(p.inlineVideo){const reference=p.key==="wedding"?external("듀엣 웨딩 필름 레퍼런스 보기","https://www.youtube.com/watch?v=5ZuTmQWCRJk","detail-reference-link"):"";return '<div class="detail-result-card">'+inlineVideoResult(p)+'<div class="detail-result-copy"><strong>'+m.result+'</strong><p>'+copy+'</p>'+reference+'</div></div>'}
...
function renderDetail(key,purpose=""){
 ...
 const result=detailResult(p,m,w,song),process=filmProduct?filmProcessSection(key,p.steps):processSection(p.steps);
 app.innerHTML='<section class="shell detail-hero"><div><h1>'+p.title+'</h1><p class="lead">'+p.sub+'</p><div class="detail-facts"><span><b>사용 시점</b>'+m.use+'</span><span><b>참여 인원</b>'+m.who+'</span></div></div>'+result+'</section>'+
 (song?arRatio():(filmProduct?filmSelectionNotice(key)+process:composition(p)))+
 (filmProduct?"":process)+
 reviews()+
...
function compactConsultationForm(p){return '<form id="consultForm" class="booking-contact"><div class="booking-contact-head"><span>예약 상담</span><strong>선택한 구성으로 바로 문의하세요</strong></div><div class="checkout-fields"><label class="checkout-field" for="contactSource"><span>알게 된 경로</span><select id="contactSource"><option value="">선택해 주세요</option>'+["인스타그램","스레드","광고","카페","블로그"].map(x=>'<option'+(consultationDraft.source===x?' selected':'')+'>'+x+'</option>').join("")+'</select></label><label class="checkout-field" for="contactName"><span>성함</span><input id="contactName" value="'+escapeHtml(consultationDraft.name)+'" placeholder="성함 입력"></label><label class="checkout-field" for="eventDate"><span>예식일 또는 사용 예정일</span><input type="date" id="eventDate" value="'+escapeHtml(consultationDraft.eventDate)+'"></label><label class="checkout-field" for="contactPurpose"><span>사용 목적</span><select id="contactPurpose"><option value="">선택해 주세요</option>'+["식전","식중","축가","기타"].map(x=>'<option'+(consultationDraft.purpose===x?' selected':'')+'>'+x+'</option>').join("")+'</select></label></div><p class="booking-availability">목요일부터 일요일까지 예약 가능 · 날짜와 시간은 상담 시 협의</p><button class="button dark" type="submit">이 구성으로 카카오톡 상담하기 '+arrow()+'</button></form>'}
function renderEvent(key,purpose=""){
 ...
 currentEventProduct=key;currentEventPurpose=purpose;if(FILM_FORMAT_PRODUCTS.has(key))selectedFilmFormat=FILM_FORMATS[purpose]?purpose:"live";const p=PRODUCTS[key];
 const options=eventProductOptions(key,purpose);
 app.innerHTML='<section class="shell section booking-head"><a class="text-link" href="'+(FILM_FORMAT_PRODUCTS.has(key)?'#/detail/'+key+'/'+selectedFilmFormat:'#/detail/'+key)+'">'+p.title+' 상품 안내 '+arrow()+'</a><h1>구성은 명확하게<br>선택은 편안하게</h1><p class="lead">포함 작업과 추가 옵션, 이벤트 혜택을 한눈에 확인해 주세요</p></section>'+
 '<div class="shell booking-layout"><div class="booking-options">'+(FILM_FORMAT_PRODUCTS.has(key)?bookingFilmFormatSection(key):"")+'<section class="package-section"><h2>기본 제작에 포함되는 작업</h2><p>선택한 영상 구성에 따라 아래 작업까지 기본으로 포함됩니다</p><ul class="package-list" data-package-list>'+bookingPackageList(key,p)+'</ul></section>'+
 '<section class="booking-extra"><h2>추가 옵션</h2><p>필요한 항목만 선택하면 예상 금액에 바로 반영됩니다</p>'+(p.category==="song"?'<div class="song-option-grid">'+SONG_OPTIONS.map(o=>'<label class="song-option-card"><img src="'+o.image+'" alt="'+o.label+' 안내 이미지" loading="lazy"><span class="song-option-select"><input type="radio" name="songOption" data-song-option="'+o.key+'" '+(chosenOption===o.key?"checked":"")+'><span><strong>'+o.label+'</strong><small>'+o.detail+'</small></span><b>'+(o.price?'+'+won(o.price):'상담 후 안내')+'</b></span></label>').join("")+'</div>'+(purpose==="friend"?'<div class="friend-options">'+options.map(renderProductOption).join("")+'</div>':""):options.map(renderProductOption).join(""))+'</section>'+
 '<details class="event-benefits"><summary><span><small>이벤트 적용가도 확인하기</small><strong>얼마나 싸질까?</strong></span><b aria-hidden="true">+</b></summary><div class="event-benefits-body"><p>해당되는 항목만 선택하면 예상 금액에서 바로 차감됩니다</p><div class="event-list">'+EVENTS.map(e=>'<label class="event-choice"><input type="checkbox" data-event="'+e.key+'" '+(selectedEvents.has(e.key)?"checked":"")+'><span><strong>'+e.label+'</strong><small>'+e.detail+'</small></span><b>−'+won(e.discount)+'</b></label>').join("")+'</div><p class="fine">이벤트 참여 조건과 혜택 적용 시점은 예약 상담에서 최종 확인합니다</p></div></details></div>'+
 '<aside class="booking-checkout" id="reservation"><div class="booking-summary" aria-live="polite" id="bookingSummary"></div>'+compactConsultationForm(p)+'</aside></div><div class="shell other-products">'+cta("다른 상품 선택하기","#/","light")+'</div>'+footer()+'<div class="booking-mobile-bar"><span id="mobilePrice"></span><button class="button dark" type="submit" form="consultForm">카카오톡 상담 '+arrow()+'</button></div>'
 updatePrice()
}
function updatePrice(){let c=calculate();const formatRow=FILM_FORMAT_PRODUCTS.has(currentEventProduct)?'<div><dt>영상 구성</dt><dd>'+FILM_FORMATS[selectedFilmFormat].title+'</dd></div>':'';const optionRows=(c.songOption?'<div><dt>'+c.songOption.label+'</dt><dd class="plus">'+(c.songOption.price?'+'+won(c.songOption.price):'상담 후 안내')+'</dd></div>':'')+c.optionEntries.map(o=>'<div><dt>'+o.label+(o.quantity>1?' '+o.quantity+'회':'')+'</dt><dd class="plus">+'+won(o.total)+'</dd></div>').join("");const eventRows=c.chosen.map(e=>'<div><dt>'+e.label+'</dt><dd class="minus">−'+won(e.discount)+'</dd></div>').join("");document.querySelector("#bookingSummary").innerHTML='<p class="eyebrow"><span class="desktop-summary-label">가격 계산</span><span class="mobile-summary-label">선택한 구성</span></p><h2>'+c.product.title+'</h2><dl class="price-lines">'+formatRow+'<div class="normal-line"><dt>정상가</dt><dd>'+won(c.product.normal)+'</dd></div>'+optionRows+eventRows+'</dl><div class="price-formula"><span><small>정상가</small>'+won(c.product.normal)+'</span><i>+</i><span><small>추가 옵션</small>'+won(c.optionPrice)+'</span><i>−</i><span><small>이벤트 혜택</small>'+won(c.discount)+'</span><i>=</i><strong><small>예상 금액</small>'+won(c.finalPrice)+'</strong></div><p class="fine">선택한 조건을 기준으로 계산한 예상 금액이며 최종 적용 여부는 상담에서 확인합니다</p>';const net=c.optionPrice-c.discount;document.querySelector("#mobilePrice").innerHTML='<span><em>정상가</em><del>'+won(c.product.normal)+'</del></span><span><em>옵션·혜택</em><small class="'+(net>0?'plus':'minus')+'">'+(net>0?'+':net<0?'−':'')+won(Math.abs(net))+'</small></span><span><em>예상 금액</em><strong>'+won(c.finalPrice)+'</strong></span>'}
function consultationText(){let c=calculate(),picked=c.optionEntries.map(o=>o.label+' +'+won(o.total));if(c.songOption)picked.unshift(c.songOption.label+(c.songOption.price?' +'+won(c.songOption.price):' · 상담 후 안내'));return ["🤍 🇼 🇪 🇱 🇨 🇴 🇲 🇪  🤍","","아래 문의 양식을 복사한 후","내용을 작성해 보내주세요 :D","","보내주신 내용을 확인한 후","최대한 빠르게 안내드리겠습니다 :)","━━━━━","","[ 𝐂𝐨𝐧𝐭𝐚𝐜𝐭 𝐅𝐨𝐫𝐦 ]","","• 어디에서 보고 오셨나요? : "+(consultationDraft.source||""),"• 성함 : "+(consultationDraft.name||""),"• 희망 서비스 : "+c.product.title,FILM_FORMAT_PRODUCTS.has(currentEventProduct)?"• 선택한 영상 구성 : "+FILM_FORMATS[selectedFilmFormat].title:null,"• 예식일, 예정일 : "+(consultationDraft.eventDate||""),"• 희망 예약일 : 상담 시 협의","• 희망 시간 : 상담 시 협의","• 상영 시점 또는 사용 목적 : "+(consultationDraft.purpose||""),"","• 선택한 추가 옵션 : "+(picked.length?picked.join(" / "):"선택 없음"),"• 이벤트 혜택 : "+(c.chosen.length?c.chosen.map(e=>e.label+" −"+won(e.discount)).join(" / "):"선택 없음"),"• 예상 금액 : "+won(c.finalPrice)].filter(x=>x!==null).join("\n")}
...
function route(){
 ...
 const home=!type||type==="section";const detail=type==="detail"&&PRODUCTS[key];const choice=type==="choose"&&FILM_FORMAT_PRODUCTS.has(key);const ar=type==="ar"&&AR_PURPOSES[key];const event=type==="event"&&PRODUCTS[key];const purpose=parts[2]||"";const validDetail=detail||choice;
...
document.addEventListener("click",e=>{
 ...
 if(el.dataset.processFormat){const section=el.closest(".film-process-section"),format=el.dataset.processFormat;section?.querySelectorAll("[data-process-format]").forEach(button=>button.setAttribute("aria-selected",String(button===el)));section?.querySelectorAll("[data-process-panel]").forEach(panel=>panel.hidden=panel.dataset.processPanel!==format)}
...
document.addEventListener("change",e=>{
 ...
 if(el.dataset.filmFormat){selectedFilmFormat=el.dataset.filmFormat;const list=document.querySelector('[data-package-list]');if(list)list.innerHTML=bookingPackageList(currentEventProduct,PRODUCTS[currentEventProduct]);updatePrice()}
...
document.addEventListener("input",e=>{const el=e.target;if(el.id==="contactName")consultationDraft.name=el.value})
